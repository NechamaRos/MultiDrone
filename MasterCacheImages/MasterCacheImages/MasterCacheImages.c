...
static void transferToStructArray(ImgInfo_t* srcArray[CACHE_SIZE], ImgInfo_t destArray[CACHE_SIZE]) ;
...
void insertBufferInToCache(SlaveImgInfo_t** slave, int slaveBufferSize)
{
    ...
    {
        imgExist = searchImgByImgIdAndSlaveId(slave[i]->imgId, slave[i]->slaveId);
...
static void transferToStructArray(ImgInfo_t* srcArray[CACHE_SIZE], ImgInfo_t destArray[CACHE_SIZE]) 
{
    for (int i = 0; i < CACHE_SIZE; ++i) {
        if (srcArray[i] != NULL) 
            destArray[i] = *srcArray[i]; // העתק את התמונה מהמצביע למערך היעד
    }
}

ImgInfo_t* searchImgByImgIdAndSlaveId(int imgId, int slaveId)
{
    ImgInfo_t imgInfo[CACHE_SIZE];
    memset(imgInfo, 0, sizeof(imgInfo));
    transferToStructArray(masterCacheImg_cb->imgArray,imgInfo);
    ImgInfo_t* find=searchByImgIdAndSlaveId(imgId,slaveId,imgInfo);
    return find?find:NULL;
}
ImgInfo_t** findImagesByRange( Point_t TL, Point_t BR)
{
    int sizeOfArrResult=0;
    ImgInfo_t imgInfo[CACHE_SIZE];
    memset(imgInfo, 0, sizeof(imgInfo));
    transferToStructArray(masterCacheImg_cb->imgArray,imgInfo);
    ImgInfo_t**ifoundimages=findImagesInRange(imgInfo,TL,BR,&sizeOfArrResult);
    return ifoundimages;
}
...
//     for (int i = 0; i < CACHE_SIZE; i++)
//     {
//         if (imgArray[i].cachePtr != NULL)
//         {
//             if (imgArray[i].imgId == imgId && imgArray[i].slaveId == slaveId)
//             {
//                 // הקצה זיכרון ל-find
...
// {
//     ImgInfo_t** found = (ImgInfo_t**)malloc(CACHE_SIZE * sizeof(ImgInfo_t*));
//     if (found == NULL) {
//         *result_size = 0;
//         return NULL;
//     }
...
//         if (imgArray[i].cachePtr != NULL)
//         {
//             if (imgArray[i].TL.x >= TL.x && imgArray[i].TL.y >= TL.y &&
//                 imgArray[i].BR.x <= BR.x && imgArray[i].BR.y <= BR.y)
//             {
//                 found[*result_size] = (ImgInfo_t*)malloc(sizeof(ImgInfo_t));
//                 if (found[*result_size] != NULL) {
//                     *found[*result_size] = imgArray[i];
//                     (*result_size)++;
//                 }
//             }
//         }
